...
typedef struct  _mc_s_eth_header
{
    uint8_t dest_mac[ETH_ALEN];	// Destination MAC address
    uint8_t src_mac[ETH_ALEN];	// Source MAC address
    uint16_t eth_type;			// EtherType (0x0806 for ARP)
}	_mc_t_eth_header;

typedef struct _mc_s_arphdr
  {
    ...
    unsigned char __ar_sip[_MC_IPV4_BYTE_SIZE];		/* Sender IP address.  */
    unsigned char __ar_tha[ETH_ALEN];	/* Target hardware address.  */
    unsigned char __ar_tip[_MC_IPV4_BYTE_SIZE];		/* Target IP address.  */
  } _mc_t_arphdr;

typedef struct  _mc_s_packet
{
        struct ethhdr        ethernet_header; 
        _mc_t_arphdr	     arp_packet;
}       _mc_t_packet;
...
typedef struct _mc_s_data
{
	...
	_mc_t_eth_header	*ethernet_header;
